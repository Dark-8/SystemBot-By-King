...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "mute")) {
    ...
    mention.addRole(role).then(() => {
      ...
      message.channel.send(
        `** ${mention.user.username}  Muted! :zipper_mouth:  **  `
      );
...
client.on("message", message => {
  ...
  if (command == prefix + "unban") {
    ...
    message.guild.fetchBans().then(bans => {
      ...
      message.channel.send(
        `: <@${userID}> **<a:overify:688707812225712152> تم فك بند العصو من السيرفر **`
      );
    });
  }
});
...
client.on("message", msg => {
  if (msg.content === "باك") {
    msg.reply("**ولكمـ نورتـ السيرفر <a:heartss:689024205584728095>**");
  }
});

client.on("message", msg => {
  if (msg.content === "هلا") {
    msg.reply("**هلااا نورت ام السيرفر <a:heartss:689024205584728095>**");
  }
});

client.on("message", msg => {
  if (msg.content === "السلام عليكم") {
    msg.reply("**<a:need:688791009706115077> وعليكم السلام**");
  }
});

client.on("message", msg => {
  if (msg.content === "On") {
    msg.reply("**Online  <a:yes:688710968821547010>**");
  }
});
...
client.on("message", message => {
  ...
  if (message.content == "#members")
    var kayan = new Discord.RichEmbed()
      ...
      .addBlankField(true)
      .addField(
        "<:online:686179966315921410>| متصل بالانترنت",
        `${
          message.guild.members.filter(m => m.presence.status == "online").size
        }`
      )
      .addField(
        "<:dnd:687387348191019017>| مشغول",
        `${message.guild.members.filter(m => m.presence.status == "dnd").size}`
      )
      .addField(
        "<:idle:687388842529259533>| وضع الهلال",
        `${message.guild.members.filter(m => m.presence.status == "idle").size}`
      )
      .addField(
        "<:off:687387125817409552>|غير متصل على الانترنت",
        `${
          message.guild.members.filter(m => m.presence.status == "offline").size
        }`
      )
      .addField(
        "<:off:687387125817409552> <:dnd:687387348191019017> <:idle:687388842529259533> <:online:686179966315921410>| اعضاء السيرفر",
...
client.on("message", async message => {
  ...
  if (message.content.startsWith(prefix + "inv")) {
    let ew = new Discord.RichEmbed()
      .setDescription(
        "[Click Here](https://discordapp.com/api/oauth2/authorize?client_id=${client.id}&permissions=8&scope=bot)"
      )
  ...
  if (command == prefix + "role") {
    ...
    if (userM) {
      ...
      if (!message.guild.member(userM.user).roles.has(getRole.id)) {
        ...
        message.channel.send(
          ` | تم اعطاء رتبه  **${getRole.name}** الي **${userM.user.tag}**`
        );
      } else if (message.guild.member(userM.user).roles.has(getRole.id)) {
        message.guild.member(userM.user).removeRole(getRole.id);
        message.channel.send(
          ` | تم ازاله رتبه  **${getRole.name}** من **${userM.user.tag}**`
        );
      }
              ...
              .then(message1 => {
                message.guild.members
                  ...
                  .forEach(m => {
                    ...
                    setTimeout(() => {
                      message1.edit(
                        `:white_check_mark: | <@${message.author.id}> تم اعطاء جميع الاشخاص رتبه **${getRole.name}** .`
                      );
                    }, 10000);
                  });
              });
                    ...
                    .then(collected => {
                      ...
                      try {
                        ...
                        message.guild.channels
                          .find("name", room)
                          .send(" <a:gifwin:688728774245744677> **Giveaway** <a:gifwin:688728774245744677>", {
                            embed: giveEmbed
                          })
                          .then(m => {
                            let re = m.react("🎉");
...
client.on("message", msg=>{
let id = "684762350938488833"; // ايديك
let role = "Ultra ★ Vip"; // اسم الرتبه
let Price = 2000; // السعر
let Price2 = 1900
...
.setTitle("لشراء رتبة اذغط على الرياكشن")
.setTimestamp()
...
if(!roleW) return msg.reply(`مقفل`)
msg.channel.send(`**لديك 4 دقائق لتحويل ${Price} كريدت UltraBot**
إلى ${msg.guild.members.get(id)}
`).then( msgs =>{

  const filter = response => response.author.id == "688694842863255568" && response.mentions._content.includes(`:moneybag: | ${msg.author.username}, has transferred \`$${Price2}\` to ${msg.guild.members.get(id)}`);
